...
Create a class Vegetable with the below attributes: name of type String ;/name of the vegetable price of type float //price ( in Rupees ) of the Fx: Kilogram quantity of type int ,/ quantity — number of units / Number of Kilograms
...
Create another class Store with the below attribute: storeName of type String /specifies the
vegList of type List L ist of Vegetable which takes all sequence. The method should set the value of attributes with the parameter values received from main.
...
fas:(‘ananas’, ‘apple), ’13: (‘banana’), ‘p’:(‘pineapple)) Here ‘a’ ,’b’ and ‘p’ (excluding the single quote ) represents the keys of dictionary and (‘ananass, ‘apple), (‘banana’), (pineapple’) ,excluding the ‘single quote and brackets’ represents the tuple for respective and corresponding keys(a , b, p ).
...
If there is no vegetable in given price range, the method returns an empty list and based on the value main function should print “No Vegetable(s) in the given price range” (Excluding the double quotes) . For more details refer the sample test case input and respective output
...
Instructions to write main and to call the methods of the classes defined above: a. You would required to write the main function completely, please follow the below instructions for the same. b. You would required to write the main program which is inlign to the “sample input description section” mentioned below and to read the data in the same sequence.
c. Create the respective objects(Vegetable and Store) with the given sequence of arguments to fulfill the constructor requirement defined in the respective class. i.Create a Vegetable object after reading the data related to Vegetable and add the Vegetable object to list of Vegetable objects, This point repeats for the number of Vegetable objects you want to create and add to Vegetable list or as mentioned in the first line of input in the input data ii. Create Store object by passing the Store name and list of Vegetable objects ( created and as mentioned in above point# c.i ) as the arguments to the constructor . d. Call the methods ( categorizeVegetablesAlphabetically and f ilterVegetablesForPriceRange) mentioned above from the main function in the same order , they appear in the question text. e. Display the data returned by the functions , in the main program as per the order of calling the functions and as per the format mentioned in the sample output. f. If None/empty list is returned.
...
class store:
    ...
    def filterVegetablesForPriceRange(self,mi,mx):
        name=[]
        for i in self.vegList:
            if i.price >=mi and i.price<=mx:
                name.append(i.name)
        return name
...
for i in range(t):
    ...
    price=float(input())
    q=int(input())
    temp=vegetable(name,price,q)
    objlist.append(temp)
storename=input()
mi=float(input())
mx=float(input())

s=store(storename,objlist)
res1=s.categorizeVegetablesAlphabetically()
...
res2=s.filterVegetablesForPriceRange(mi,mx)

if res2==[]:
    print("No Vegetable found for given prince range")
else:
    print(str(mi)+"-"+str(mx))
